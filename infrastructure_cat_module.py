...
def init_local_pivot_graph(iris_results: list, config: "Config"):
    ...
    for domain_json in iris_results:
        ...
        append_value_with_count(graph, 'adsense', domain_json, domain, config)
        append_value_with_count(graph, 'google_analytics', domain_json, domain, config)
        append_value_with_count(graph, 'create_date', domain_json, domain, config)
        append_value_with_count(graph, 'redirect_domain', domain_json, domain, config)
        append_value_with_count(graph, 'registrar', domain_json, domain, config)

        # haven't seen "ssl_email" in the wild yet, so not sure if it is a value/count or just value 
        append_values_with_counts(graph, 'ssl_email', domain_json, domain, config)
        ...
        for ip_json in domain_json["ip"]:
            ...
            append_value_with_count(graph, 'address', ip_json, domain, config, 'ip_address')
            append_value_with_count(graph, 'country_code', ip_json, domain, config, 'ip_country_code')
            append_value_with_count(graph, 'isp', ip_json, domain, config, 'ip_isp')
            append_values_with_counts(graph, 'asn', ip_json, domain, config, 'ip_asn')

        # name servers are composite objects, so pull out each value for each name server
        for ns_json in domain_json["name_server"]:
            append_value_with_count(graph, 'host', ns_json, domain, config, 'ns_host')
            append_value_with_count(graph, 'domain', ns_json, domain, config, 'ns_domain')
            append_values_with_counts(graph, 'ip', ns_json, domain, config, 'ns_ip')

        append_value(graph, 'tld', domain_json, domain, config)

        # ssl certs are composite objects, so pull out each value for each ssl cert
        for ssl_json in domain_json['ssl_info']:
            append_value_with_count(graph, 'hash', ssl_json, domain, config, "ssl_hash")
            append_value_with_count(graph, 'subject', ssl_json, domain, config, "ssl_subject")
            append_value_with_count(graph, 'organization', ssl_json, domain, config, "ssl_org")

        # mx servers are composite objects, so pull out each value for each mx server
        for mx_json in domain_json['mx']:
            append_value_with_count(graph, 'host', mx_json, domain, config, "mx_host")
            append_value_with_count(graph, 'domain', mx_json, domain, config, "mx_domain")
            append_values_with_counts(graph, 'ip', mx_json, domain, config, "mx_ip")
            # mx priority might be interesting at some point to node stringth  
    return graph, domains
    

def append_value(graph: "Graph",
                 pivot_category: str,
                 json_data: dict,
                 domain: "Domain",
                 config: "Config",
    ...
    if pivot_category in json_data:
        ...
        if len(pivot_value) > 0:
            _append_value_to_pivot(graph, pivot_category, pivot_value, None,
                                   domain, config, new_pivot_category)

            
def append_value_with_count(graph: "Graph",
                            pivot_category: str,
                            json_data: dict,
                            domain: "Domain",
                            config: "Config",
        ...
        if isinstance(json_data[pivot_category], dict): 
            ...
            if global_pivot_count < config.global_count_threshold or pivot_category == "create_date":
                ...
                if len(pivot_value) > 0 and global_pivot_count > 0:
                    _append_value_to_pivot(graph, pivot_category, pivot_value,
                                           global_pivot_count, domain, config, new_pivot_category)
            
            
def append_values_with_counts(graph: "Graph",
                              pivot_category: str,
                              json_data: dict,
                              domain: "Domain",
                              config: "Config",
...
def _append_value_to_pivot(graph: "Graph",
                           pivot_category: str,
                           pivot_value: str,
...
#         pivot_categories[pivot_category][pivot_value] = Pivot(pivot_value, global_pivot_count)

    # add pivot to the graph
    pivot = Pivot(pivot_category, pivot_value, global_pivot_count)
    if graph.has_node(pivot.label()):
        pivot = graph.nodes[pivot.label()]["pivot"]
    else:
        graph.add_node(pivot.label(), pivot=pivot)    
    pivot.domains.add(domain) # maybe just the domain name...not sure yet
    # TURBO: not sure i want this two way relationship? it makes it to messy when deleting a pivot or domain. you have to go clean up the other half of the relationship
    domain.pivots[pivot.label()] = pivot
...
def trim_pivots(graph: "Graph", domain_count: int, config: "Config"):
    ...
    del_min_count = 0
    del_all_count = 0
    pivot_nodes = list(graph.nodes)
    for node in pivot_nodes:
        pivot = graph.nodes[node]["pivot"]
        if len(pivot.domains) < config.min_pivot_size:
            # i need a better way to clean up domain to pivot relationships
            for domain in pivot.domains:
                domain.pivots.pop(node)
            graph.remove_node(node)
            del_min_count += 1
        elif len(pivot.domains) >= domain_count:
            # i need a better way to clean up domain to pivot relationships
            for domain in pivot.domains:
                domain.pivots.pop(node)
            graph.remove_node(node)
    ...
    if config.print_debug_output:
        print(f"deleted {del_min_count} pivots for being smaller than min_pivot_size "
              f"and {del_all_count} pivots for containing all domains in result set "
              f"out of {len(pivot_nodes)} total pivots")
    return graph
...
def trim_unconnected_domains(graph: "Graph", domains: dict, config: "Config"):
    ...
    for node in graph.nodes:
        pivot_domains = graph.nodes[node]["pivot"].domains
...
#             connected_domains = connected_domains.union(pivot_domains)
    ...
    distinct_domains = set(domains.keys())
    lonely_domains = distinct_domains.difference(connected_domains)
...
def build_local_pivot_graph(graph: "Graph", domains: dict, config: "Config"):
    # The graph in initialized with all it's nodes. Now we need to connect all the nodes with each edge
    edge_count = 0    
    for domain in domains:
        ...
        for x in range(len(pivots)):
            for y in range(x+1, len(pivots)):
                ...
                if graph.has_edge(p1, p2):
                    # graph[p1][p2] #['relationship'].add(weight, category)
                    pass
                else:
                    graph.add_edge(p1, p2) #, relationship=DomainRelationship(weight, category))
...
def build_3d_graph_layout(graph: "Graph"):
    ...
    # build a random color map for now
    color_map={}
    def get_color(category):
        if not category in color_map:
            color_map[category] = list(np.random.choice(range(256), size=3))
    ...
    for node in graph.nodes:
        ...
        pivot = graph.nodes[node]["pivot"]
        node_labels.append(pivot.label())        
        node_risk_scores.append(get_color(pivot.category))
